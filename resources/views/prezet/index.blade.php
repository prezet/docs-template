<x-prezet.template :nav="$nav" :headings="[]">
    ...
    <section class="relative min-w-0 max-w-6xl">
        {{-- Header Section --}}
        <div class="py-10 border-b border-slate-200 sm:py-12 dark:border-slate-800">
            <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                <div class="space-y-2">
                    <h1 class="font-display text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl dark:text-white">
                        Documentation
                    </h1>
                    <p class="max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-400">
                ...
                @if ($currentTag || $currentCategory)
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @if($currentTag || $currentCategory)
                            <a href="{{ route('prezet.index') }}" class="ml-1 text-xs font-medium text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300">Clear all</a>
                        @endif
                    </div>
                @endif
            </div>
        ...
        <ul class="divide-y divide-slate-200 dark:divide-slate-800">
            @forelse ($articles as $article)
                <li class="py-10 transition-opacity duration-200 hover:opacity-90">
            ...
            @empty
                <li class="py-24 text-center">
                    ...
                    <p class="mt-1 text-slate-500 dark:text-slate-400">We couldn't find anything matching your criteria.</p>
                    <div class="mt-6">
